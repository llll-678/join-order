...
//#include "params.h"             /* RELATION_PADDING for Parallel Radix */
/** from params.h */
/** @warning This padding must be allocated at the end of relation */
#define RELATION_PADDING (24 * 128 * 2 * sizeof(int64_t))
#ifndef CACHE_LINE_SIZE
#define CACHE_LINE_SIZE 64
#endif
#define PADDINGX8 (128 * CACHE_LINE_SIZE/sizeof(tuple_t))

/* return a random number in range [0,N] */
#define RAND_RANGE(N) ((double)rand() / ((double)RAND_MAX + 1) * (N))
#define RAND_RANGE48(N,STATE) ((double)nrand48(STATE)/((double)RAND_MAX+1)*(N))
#define MALLOC(X) malloc(X + RELATION_PADDING)
...
alloc_aligned(size_t size)
{
    ...
    //printf("===> alloc_aligned() = size=%d, numa-localize=%d\n", size, numalocalize);
    if (rv) { 
...
create_relation_pk(relation_t *relation, int64_t num_tuples) 
{
    ...
    rv = posix_memalign((void**)&relation->tuples,  64, num_tuples * sizeof(tuple_t));
    
    if (rv && !relation->tuples) { 
                         ...
                         uint32_t nthreads, uint64_t maxid) 
{
    ...
    /* we need aligned allocation of items */
    rv = posix_memalign((void**)&relation->tuples,  CACHE_LINE_SIZE,
                        num_tuples * sizeof(tuple_t) 
                        + RELATION_PADDING + nthreads*PADDINGX8*sizeof(tuple_t));
                              ...
                              const int64_t maxid) 
{
    ...
    numa_localize(relation->tuples, relation->num_tuples, nthreads);
                     ...
                     const int64_t maxid, const double zipf_param) 
{
    ...
    relation->num_tuples = num_tuples;
    //relation->tuples = (tuple_t*)MALLOC(relation->num_tuples * sizeof(tuple_t));
    int rv = posix_memalign((void**)&relation->tuples,  CACHE_LINE_SIZE, sz);
    
    if (rv && !relation->tuples) { 
        perror("[ERROR] Out of memory");
        return -1; 
    }
    /* make sure the relation is NUMA-aware placed*/
    /* printf("\n[INFO ] Localizing the relation NUMA-aware.. (nthr=%d)\n", nthreads); */
    numa_localize(relation->tuples, relation->num_tuples, nthreads);
