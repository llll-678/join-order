...
do
  ...
  # 获取初始内存使用情况
        ...
        # 启动程序并监控内存使用
         ./src/mchashjoins --r-size=$r_size --s-size=$s_size -a NPO -n 96 > tmp.log &  # 在后台启动程序
        PROGRAM_PID=$!
        ...
        echo "INDEX: $INDEX, TOTAL-TIME-USECS: $time_use, TOTAL_MEM_USE_MB: $DIFF, MEM_UTILIZATION: $mem_utilization" >> npo-rs-result.log
